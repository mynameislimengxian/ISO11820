...
/// <summary>
/// 数据库初始化器 — 程序首次运行时自动建库建表并插入初始数据。
/// 所有 SQL 使用 IF NOT EXISTS / WHERE NOT EXISTS，可安全重复调用。
/// </summary>
public class DatabaseInitializer
{
    ...
    /// <summary>
    /// 执行完整初始化：建表 6 张 + 索引 5 条 + 初始数据 20 条。
    /// 如果数据库已存在且表已建好，不会重复执行。
    /// </summary>
    ...
    private static void CreateTables(SqliteConnection conn)
    {
        ...
        // 6. CalibrationRecords（⚠️ 表名 PascalCase）
        Execute(conn, @"
    ...
    {
        Execute(conn, "CREATE INDEX IF NOT EXISTS \"IX_Testmaster_Testdate\" ON \"testmaster\" (\"testdate\")");
        Execute(conn, "CREATE INDEX IF NOT EXISTS \"IX_Testmaster_Operator\" ON \"testmaster\" (\"operator\")");
        Execute(conn, "CREATE INDEX IF NOT EXISTS \"IX_Testmaster_Testdate_Productid\" ON \"testmaster\" (\"testdate\", \"productid\")");
        Execute(conn, "CREATE INDEX IF NOT EXISTS \"IX_CalibrationRecord_Date\" ON \"CalibrationRecords\" (\"CalibrationDate\")");
        Execute(conn, "CREATE INDEX IF NOT EXISTS \"IX_CalibrationRecord_Operator\" ON \"CalibrationRecords\" (\"Operator\")");
    }
    ...
    private static void SeedData(SqliteConnection conn)
    {
        ...
        // sensors（17 条：0~16）
        SeedSensor(conn, 0, "Sensor0", "炉温1", "采集", "炉温1");
        SeedSensor(conn, 1, "Sensor1", "炉温2", "采集", "炉温2");
        SeedSensor(conn, 2, "Sensor2", "表面温度", "采集", "表面温度");
        SeedSensor(conn, 3, "Sensor3", "中心温度", "采集", "中心温度");
        for (int i = 4; i <= 15; i++)
            SeedSensor(conn, i, $"Sensor{i}", $"备用通道{i + 1}", "采集", $"备用通道{i + 1}");
        SeedSensor(conn, 16, "Sensor16", "校准温度", "校准", "校准温度");
    }

    private static void SeedSensor(SqliteConnection conn, int id, string name,
        string dispName, string group, string desc)
    {
